        ...
        switch(UartMode.State)
        {
            ...
            case 2 ://位置规划准备
            {
                if(UartMode.CMD == CMD_Start) //准备进行位置规划
                {
                    ...
                    PostionPlanCiA402Mode_1.a_accel = 0.06;       //PostionPlan_a_accel ;
                    PostionPlanCiA402Mode_1.a_decel = 0.06;       //PostionPlan_a_decel;
                    PostionPlanCiA402Mode_1.decel_max = 0.008;    //PostionPlan_decel_max;
					
                    //目标速度设置
                    PostionPlan_vel_tar = 150;
             ...
             case 3 :  //电机正在运行过程中
            {
                ...
                {	int target=MotorControler.MotorActivePostion;
					UartMode.State = 4;
					UqsPid.Ui=0;
					MotorControler.State = 0; //下使能
					delay_Ms(20);					
    ...
    if(UartMode.Mode == Torque_Mode) //转矩控制扭矩控制模式
    {
        ...
        switch(UartMode.State)
        {
            ...
            case 2 ://电机正在运行过程中
            {
				...
				speed_err=abs(100-MotorControler.SpeedFdbp);
				
				
				if((last_speed_ref!=MotorControler.SpeedRef)||(speed_err>50))
				{
					if(speed_err>3)
					{
						pidpv.Ki = 6*pidpv_Ki;
					...
					{
						last_speed_ref=MotorControler.SpeedRef;
					}
					
				}
				if((pidpv.Ref!=0)&&(MotorControler.SpeedFdbp==0))
				{
					pidpv.Ki = 20*pidpv_Ki;
				}
				...
				if((UartMode.Torque > 0)&&(UartMode.Torque < 40))
                {
                    UartMode.Torque = 40;
                }
				if((UartMode.Torque < 0)&&(UartMode.Torque > -40))
        ...
        switch(UartMode.State)
        {
            ...
            {	
             //速度环PID
				...
				speed_err=abs(MotorControler.SpeedRef-MotorControler.SpeedFdbp);
				
				
				if(last_speed_ref!=MotorControler.SpeedRef)
				{
					if(speed_err>3)
					{
						pidpv.Ki = 300;
