...
SP_Error Set_SpeedPlant_Para(Set_SP_Para *ptr)
{
    ...
    if(ptr->a_decel < 0)
    {
		...
		int vel_tar_abs = _IQ15abs(ptr->vel_tar);

    if(vel_tar_abs < _IQ15(1)) //定点,速度不能小于1
    {
    ...
    if(ptr->end_position < ptr->start_position)
    {
        ...
        ptr->vel_tar = -vel_tar_abs; //目标转动方向一定要跟目标速度同号
    }
    else
    {
        sp->forward_flag = 1;
        ptr->vel_tar = vel_tar_abs;
    }
    ...
    {
        if((-ptr->vel_init) <= ptr->vel_tar) //Vo<Vm<0
        {
            sp->vel_flag = -1;
    ...
    int accel_max_tem = ptr->accel_max; //存储最大加速度，用于后面最大加速度判断

    if(sp_error == none_err)
    {
        //这样可以将情况整合在一起，所有情况都能到达最高加速度，只是这种情况下的t2=t1
        if(_IQ15abs(ptr->vel_tar - ptr->vel_init) <= _IQ15div(_IQ15mpy(accel_max_tem, accel_max_tem),ptr->a_accel))
        {
            //设置的最大速度较小。很快到达，前半段没有匀加速度阶段，将最大加速度重置为临界加速度
            ptr->accel_max = _IQ15sqrt(_IQ15mpy(_IQ15abs(ptr->vel_tar - ptr->vel_init),ptr->a_accel));
        }

        if(_IQ15abs(ptr->vel_tar) <= _IQ15div(_IQ15mpy(ptr->decel_max, ptr->decel_max),ptr->a_decel))//与上面同理
        {
            ptr->decel_max = _IQ15sqrt(_IQ15mpy(_IQ15abs(ptr->vel_tar),ptr->a_decel));
        }

				int tmp = _IQdiv2(_IQ15div(ptr->accel_max,ptr->a_accel) + _IQ15div(_IQ15abs(ptr->vel_tar - ptr->vel_init), ptr->accel_max));
				tmp = _IQ30mpy(ptr->vel_tar + ptr->vel_init,tmp);
        sp->s7 = tmp + _IQ30mpy(ptr->vel_tar,_IQdiv2(_IQ15div(ptr->decel_max,ptr->a_decel) + _IQ15div(ptr->vel_tar,ptr->decel_max)));

        //目标位置不足以走完没有匀速阶段的全程
        while((ptr->end_position - ptr->start_position) < sp->s7)
        {
            //如果目标速度与初始速度相同时仍不能实现曲线，则降低目标速度也不能实现，提示出错
            if(_IQ15abs(ptr->vel_tar - ptr->vel_init) < _IQ15(1))
            {
                sp_error = distance_too_small_err;
                break;
            }

            ptr->vel_tar = _IQ15mpy(ptr->vel_tar,_IQ15(0.95)); //降低目标速度

            //如果目标速度过低，认为曲线无法实现
            if(ptr->vel_tar < _IQ15(1))  //由于速度是定点，至少为1
            {
                sp_error = distance_too_small_err;
                break;
            }
            ...
            //重新计算总位移
            tmp = _IQdiv2(_IQ15div(ptr->accel_max,ptr->a_accel) + _IQ15div(_IQ15abs(ptr->vel_tar - ptr->vel_init), ptr->accel_max));
						tmp = _IQ30mpy(ptr->vel_tar + ptr->vel_init,tmp);
						sp->s7 = tmp + _IQ30mpy(ptr->vel_tar,_IQdiv2(_IQ15div(ptr->decel_max,ptr->a_decel) + _IQ15div(ptr->vel_tar,ptr->decel_max)));
        }
        ...
        //计算有匀速阶段的曲线数据
        int t1 = _IQ15div(ptr->accel_max,ptr->a_accel); 
        int t2;
        if(ptr->accel_max < _IQ15(0.0001)) 
        {
            t2 = t1; //无匀加速,t2和t1重合
        }
        else
        {
            t2 = _IQ15div(_IQ15abs(ptr->vel_tar - ptr->vel_init),ptr->accel_max);
        }

        int t5_t4 = _IQ15div(ptr->decel_max,ptr->a_decel);
        int t6_t4;
        if(ptr->decel_max < _IQ15(0.0001)) 
        {
            t6_t4 = t5_t4; //无匀减速,t6和t5重合
        }
        else
        {
            t6_t4 = _IQ15div(ptr->vel_tar,ptr->decel_max);
        }

        int t2_t1 = t2 - t1;
        int t6_t5 = t6_t4 - t5_t4;
				int direction = sp->vel_flag;
        sp->t1 = t1;
        sp->t2 = t2;
        sp->t3 = t2 + t1;
				int_fast64_t delta = ((int_fast64_t)(ptr->end_position - sp->s7))  << 15; //如果直接用_IQ15放大，可能会溢出
        sp->t4 = sp->t3 + _IQ15((int)( delta / ptr->vel_tar));
        sp->t5 = sp->t4 + t5_t4;
        sp->t6 = sp->t4 + t6_t4;
        sp->t7 = sp->t6 + t5_t4;

        sp->v1 = ptr->vel_init + direction * _IQdiv2(_IQ15mpy(ptr->a_accel,_IQ15mpy(t1, t1)));
        sp->v2 = sp->v1 + direction * _IQ15mpy(ptr->accel_max,t2_t1);
        sp->v3 = ptr->vel_tar;
        sp->v4 = sp->v3;
        sp->v5 = sp->v4 - direction * _IQdiv2(_IQ15mpy(ptr->a_decel,_IQ15mpy(t5_t4, t5_t4)));
        sp->v6 = sp->v5 - direction * _IQ15mpy(ptr->decel_max,t6_t5);
				
        //由于位移是以count为单位，只有整数，不需要化成IQ格式
				tmp = direction * _IQ30mpy(_IQ15div(ptr->a_accel,_IQ15(6)),_IQ15mpy(_IQ15mpy(t1,t1),t1)); 
        sp->s1 = ptr->start_position + _IQ30mpy(ptr->vel_init,t1) + tmp;
        sp->s2 = sp->s1 + _IQ30mpy(sp->v1,t2_t1) + direction * _IQ30mpy(_IQdiv2(ptr->accel_max),_IQ15mpy(t2_t1,t2_t1));
        sp->s3 = sp->s2 + _IQ30mpy(sp->v2,t1) + direction * _IQ30mpy(_IQdiv2(ptr->accel_max),_IQ15mpy(t1,t1)) - tmp;
        sp->s4 = sp->s3 + _IQ30mpy(sp->v3, sp->t4 - sp->t3);
				tmp = direction * _IQ30mpy(_IQ15div(ptr->a_decel,_IQ15(6)),_IQ15mpy(_IQ15mpy(t5_t4,t5_t4),t5_t4)); 
        sp->s5 = sp->s4 + _IQ30mpy(sp->v4,t5_t4) - tmp;
        sp->s6 = sp->s5 + _IQ30mpy(sp->v5,t6_t5) - direction * _IQ30mpy(_IQdiv2(ptr->decel_max),_IQ15mpy(t6_t5,t6_t5));
...
Current_value SpeedPlant_positionControl(Set_SP_Para* ptr, int t)
{
    ...
    {
        int t_pow = _IQ15mpy(t,t);
        current_value.accel = sp->vel_flag * _IQ15mpy(ptr->a_accel,t);
        current_value.vel = ptr->vel_init + sp->vel_flag * _IQ15mpy(_IQdiv2(ptr->a_accel),t_pow);
        current_value.position = ptr->start_position + sp->vel_flag * _IQ30mpy(_IQ15mpy(_IQ15div(ptr->a_accel,_IQ15(6)),t_pow),t) + _IQ30mpy(ptr->vel_init,t);
    }
    else if(t <= sp->t2)
    {
        t = t - sp->t1;
        current_value.accel = sp->vel_flag * ptr->accel_max;
        current_value.vel = sp->v1 + _IQ15mpy(current_value.accel,t);
        current_value.position = sp->s1 + _IQ30mpy(sp->v1,t) + sp->vel_flag * _IQ30mpy(_IQdiv2(ptr->accel_max),_IQ15mpy(t, t));
    }
    else if(t <= sp->t3)
    {
        t = t - sp->t2;
        int t_pow =  _IQ15mpy(t,t);
			  int dir = sp->vel_flag;
        current_value.accel = dir * (ptr->accel_max - _IQ15mpy(ptr->a_accel,t));
        current_value.vel = sp->v3 - dir * _IQ15mpy(_IQdiv2(ptr->a_accel),_IQ15mpy(sp->t1 - t, sp->t1 - t));
        current_value.position = sp->s2 + _IQ30mpy(sp->v2,t) + dir * _IQ30mpy(_IQdiv2(ptr->accel_max),t_pow) - dir * _IQ30mpy(_IQ15div(ptr->a_accel,_IQ15(6)),_IQ15mpy(t_pow,t));
    }
    else if(t <= sp->t4)
    {
        current_value.accel = 0;
        current_value.vel = sp->v3;
        current_value.position = sp->s3 + _IQ30mpy(sp->v3,(t - sp->t3));
    }
    else if(t <= sp->t5)
    {
        t = t - sp->t4;
        int t_pow = _IQ15mpy(t,t);
        current_value.accel = -_IQ15mpy(ptr->a_decel,t);
        current_value.vel = sp->v4 - _IQ15mpy(_IQdiv2(ptr->a_decel),t_pow);
        current_value.position = sp->s4 + _IQ30mpy(sp->v4,t) - _IQ30mpy(_IQ15div(ptr->a_decel,_IQ15(6)),_IQ15mpy(t_pow,t));
    }
    else if(t <= sp->t6)
    {
        t = t - sp->t5;
        current_value.accel = -ptr->decel_max;
        current_value.vel = sp->v5 - _IQ15mpy(ptr->decel_max,t);
        current_value.position = sp->s5 + _IQ30mpy(sp->v5,t) - _IQ30mpy(_IQdiv2(ptr->decel_max),_IQ15mpy(t,t));
    }
    else if(t <= sp->t7)
    {
        t = t - sp->t6;
        int t_pow = _IQ15mpy(t,t);
        current_value.accel = _IQ15mpy(ptr->a_decel,t) - ptr->decel_max;
        current_value.vel = sp->v6 - _IQ15mpy(ptr->decel_max,t) + _IQ15mpy(_IQdiv2(ptr->a_decel),t_pow);
        current_value.position = sp->s6 + _IQ30mpy(sp->v6,t) - _IQ30mpy(_IQdiv2(ptr->decel_max),t_pow) + _IQ30mpy(_IQ15div(ptr->a_decel,_IQ15(6)),_IQ15mpy(t_pow,t));
    }
    else if(t > sp->t7)
    {
        current_value.accel = 0;
...
//ptr用于设定初始参数，tar_ptr用于设定多段参数，n用于指示段数，上面的sp_ptr用于存放多段曲线数据
/// @brief 根据ptr指针和目标参数指针tar_ptr计算多段曲线的参数
/// @param ptr 指向存储初始参数的Set_SP_Para结构体
/// @param tar_ptr 指向存储多段的目标速度、结束速度、目标位置的内存的首个Set_SP_Para_multi结构体
/// @param n 为S型曲线的段数
//SP_Error Set_SpeedPlant_Para_multi(Set_SP_Para *ptr, Set_SP_Para_multi *tar_ptr,int n)
//{
//    SP_Error sp_error=none_err;
//		int i;
//    float position_tem=0;
//    step_num=n;
//    sp_ptr = (struct SpeedPlant_Multi *)malloc((unsigned int)step_num * sizeof(struct SpeedPlant_Multi));
//    //检查输入数据合法性
//    if(ptr->accel_max<0){ sp_error=accel_max_neg_err; }
//    if(ptr->decel_max<0){ sp_error=decel_max_neg_err; }
//    if(ptr->a_accel<0){ sp_error=a_accel_neg_err; }
//    if(ptr->a_decel<0){ sp_error=a_decel_neg_err; }
//    accel_max_tem_multi=(*ptr).accel_max;
//    decel_max_tem_multi=(*ptr).decel_max;

//    for(i=0;i<n;i++){
//        //目标速度不能为0
//        if(fabsf((*(tar_ptr+i)).vel_tar)<1e-6f){
//            sp_error=vel_tar_zero_err;
//            multi_err_num=i+1;
//            return sp_error;
//        }
...
//        (*(sp_ptr+i)).para=*ptr;
//        if(i==0){
//            (*(sp_ptr+i)).vel_end=(*(tar_ptr+i)).vel_end;
//            (*(sp_ptr+i)).para.end_position=(*(tar_ptr+i)).end_position;
//            (*(sp_ptr+i)).para.vel_tar=(*(tar_ptr+i)).vel_tar;
//        }
//        else{
//            (*(sp_ptr+i)).para.start_position=(*(sp_ptr+i-1)).para.end_position;
//            (*(sp_ptr+i)).para.vel_init=(*(sp_ptr+i-1)).vel_end;
//            (*(sp_ptr+i)).para.end_position=(*(tar_ptr+i)).end_position;
//            (*(sp_ptr+i)).para.vel_tar=(*(tar_ptr+i)).vel_tar;
//            (*(sp_ptr+i)).vel_end=(*(tar_ptr+i)).vel_end;
//        }
...
//    //这里不能与前面的赋值共用一个循环，因为下面的判断在倒转时会改变vel_end等参数
//    for(i=0;i<n;i++){
//        //倒转判断
//        if((*(sp_ptr+i)).para.end_position<(*(sp_ptr+i)).para.start_position){
//            (*(sp_ptr+i)).forward_flag = -1;
//            position_tem=(*(sp_ptr+i)).para.start_position;
//            (*(sp_ptr+i)).para.start_position = (*(sp_ptr+i)).para.end_position;
//            (*(sp_ptr+i)).para.end_position = position_tem;
//            (*(sp_ptr+i)).para.vel_init = -(*(sp_ptr+i)).para.vel_init;
//            (*(sp_ptr+i)).vel_end = -(*(sp_ptr+i)).vel_end;
//            (*(sp_ptr+i)).para.vel_tar = -fabsf((*(sp_ptr+i)).para.vel_tar); //目标转动方向一定要跟目标速度同号
//        }
//        else{
//            (*(sp_ptr+i)).forward_flag =1;
//            (*(sp_ptr+i)).para.vel_tar = fabsf((*(sp_ptr+i)).para.vel_tar);
//        }
//        //速度方向判断
//        (*(sp_ptr+i)).vel_flag_f=1;
//        (*(sp_ptr+i)).vel_flag_b=1;
//        if(((*(sp_ptr+i)).para.vel_tar>=0)&&((*(sp_ptr+i)).para.vel_init>=(*(sp_ptr+i)).para.vel_tar)) //Vo>=Vm>=0
//        {
//            (*(sp_ptr+i)).vel_flag_f=-1;
//        }
//        else if((*(sp_ptr+i)).para.vel_tar<0) //Vm<0
//        {
//            if((-1.0*(*(sp_ptr+i)).para.vel_init)<=(*(sp_ptr+i)).para.vel_tar){ //Vo<Vm<0
//                (*(sp_ptr+i)).vel_flag_f=-1;
//            }
//            if((-1.0*(*(sp_ptr+i)).vel_end)<=(*(sp_ptr+i)).para.vel_tar){ //Vo<Vm<0
//                (*(sp_ptr+i)).vel_flag_b=-1;
//            }
//            (*(sp_ptr+i)).para.vel_tar=-(*(sp_ptr+i)).para.vel_tar;
//        }
//        if(((*(sp_ptr+i)).para.vel_tar>=0)&&((*(sp_ptr+i)).vel_end>=(*(sp_ptr+i)).para.vel_tar))
//        {
//            (*(sp_ptr+i)).vel_flag_b=-1;
//        }
//        if(sp_error==none_err)
//        {
//            //计算没有匀速阶段的时间和路程数据
//            sp_cal_multi(sp_ptr+i,0); //wjj false
//            //目标位置不足以走完没有匀速阶段的全程
//            if((*(sp_ptr+i)).para.end_position<(*(sp_ptr+i)).s7)
//            {
//                sp_error=distance_too_small_err;
//                multi_err_num=i+1;
//            }
//            else{sp_cal_multi(sp_ptr+i,1);}//计算有匀速阶段的曲线t4及之后的数据  //true
//        }
//    }
//    return sp_error;
//}
